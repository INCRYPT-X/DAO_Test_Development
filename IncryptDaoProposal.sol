...
interface IIncryptXDAOMembers {
  struct Data {
    address payable A_BSCWalletAddress;
  }
  function Exists(address recordId) external returns(bool);
  function GetById(address recordId) external returns(uint256,IIncryptXDAOMembers.Data memory);
}
...
contract dtable is owned {
  ...
  struct Data {
    ...
    uint A_VotesAgainst;
    uint A_Created;
    address A_IncryptXDAOMembers_Pointer;
  }
  struct Record {
    Data data;
    uint idListPointer;
  ...
  string public Name = "DAOProposals";
  ...
  function GetByIndex(uint recordIndex) public returns(address recordId, Data memory record ,IIncryptXDAOMembers.Data memory R_IncryptXDAOMembers_Data) {
    require(recordIndex < IdList.length, 'recordIndex-out-of-range');
        IIncryptXDAOMembers A_IncryptXDAOMembers_Pointer_pointerInstance = IIncryptXDAOMembers(0xd59fEBc6a7798e675Db56e32069de55765B15B24);
        IIncryptXDAOMembers.Data memory IncryptXDAOMembers_Data;
        uint A_IncryptXDAOMembers_Pointer_index;
        (A_IncryptXDAOMembers_Pointer_index, IncryptXDAOMembers_Data) = A_IncryptXDAOMembers_Pointer_pointerInstance.GetById(Table[IdList[recordIndex]].data.A_IncryptXDAOMembers_Pointer);
      return (IdList[recordIndex], Table[IdList[recordIndex]].data, IncryptXDAOMembers_Data);
  }

  function GetById(address recordId) public returns(uint index, Data memory record ,IIncryptXDAOMembers.Data memory R_IncryptXDAOMembers_Data) {
    require(Exists(recordId), 'recordId-not-found');
        IIncryptXDAOMembers A_IncryptXDAOMembers_Pointer_pointerInstance = IIncryptXDAOMembers(0xd59fEBc6a7798e675Db56e32069de55765B15B24);
        IIncryptXDAOMembers.Data memory IncryptXDAOMembers_Data;
        uint A_IncryptXDAOMembers_Pointer_index;
        (A_IncryptXDAOMembers_Pointer_index, IncryptXDAOMembers_Data) = A_IncryptXDAOMembers_Pointer_pointerInstance.GetById(Table[recordId].data.A_IncryptXDAOMembers_Pointer);
      return (Table[recordId].idListPointer, Table[recordId].data, IncryptXDAOMembers_Data);
  }
  ...
  function Insert(Data memory recordData) public authorized returns(bool success) {
    ...
    IIncryptXDAOMembers A_IncryptXDAOMembers_Pointer_pointerInstance = IIncryptXDAOMembers(0xd59fEBc6a7798e675Db56e32069de55765B15B24);
    require(A_IncryptXDAOMembers_Pointer_pointerInstance.Exists(recordData.A_IncryptXDAOMembers_Pointer), 'pointer-record-address-not-found');
